...
function capturePageSelectors(url,scenarios,viewports,bitmaps_reference,bitmaps_test,isReference){
  ...
  casper.each(scenarios,function(casper, scenario, scenario_index){
    ...
    casper.each(viewports, function(casper, vp, viewport_index) {
      ...
      var url = scenario[1];
      if (isReference && scenario[2]) {
        url = scenario[2];
      }

      var onBeforeScript = scenario[9] || config.onBeforeScript;
      if (onBeforeScript) {
      ...
      this.thenOpen(url, function() {
        casper.waitFor(
          function(){ //test
            var readyEvent = scenario[6] || config.readyEvent;
            if(!readyEvent) {
              return true;
            }
          ...
          ,function(){//on done
            consoleBuffer = '';
            casper.echo('Ready event has received.');
          }
          ,function(){casper.echo('ERROR: casper timeout.')} //on timeout
          ,scriptTimeout
        );
        casper.wait(scenario[7]||1);
      });
      ...
      casper.then(function() {
        ...
        // };
        //
        var onReadyScript = scenario[10] || config.onReadyScript;
        if (onReadyScript) {
      ...
      selectors = scenario[5].split("|");
        ...
        //CREATE SCREEN SHOTS AND TEST COMPARE CONFIGURATION (CONFIG FILE WILL BE SAVED WHEN THIS PROCESS RETURNS)
         ...
         selectors.forEach(function(o,i,a){
          ...
          if (!isReference) {
            compareConfig.testPairs.push({
              ...
              fileName:fileName,
              label:scenario[0],
              misMatchThreshold: scenario[8] || config.misMatchThreshold
            });
          }
